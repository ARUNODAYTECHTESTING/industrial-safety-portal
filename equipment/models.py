...
class Plant(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = Plant.objects.filter().first()
            if obj:
                self.id = obj.id + 1
...
class Station(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = Station.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class EquipmentType(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = EquipmentType.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class Equipment(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = Equipment.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class ScheduleType(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = ScheduleType.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class Schedule(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = Schedule.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class MasterAuditParameter(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = MasterAuditParameter.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class Checkpoint(shared_models.TimeStamp):
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = Checkpoint.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
...
class Audit(shared_models.TimeStamp):
    ...
    correction_image = models.ImageField(upload_to = 'audit/',null=True,blank=True)
...
class Observation(shared_models.TimeStamp):
    ...
    audit_image = models.ImageField(upload_to = 'observation/',null=True,blank=True)
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            obj = Observation.objects.filter().first()
            if obj:
                self.id = obj.id + 1
        super().save(*args, **kwargs)
